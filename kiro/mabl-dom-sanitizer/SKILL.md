...
metadata:
  ...
  host: kiro
compatibility: >-
  Requires Python 3 (standard library only). No network access needed.
---
...
Paths in the command above are relative to this skill's folder. If you invoke it
from elsewhere, use the absolute path — e.g.
`python3 ~/.kiro/skills/mabl-dom-sanitizer/scripts/sanitize_dom_snapshot.py …`.
...
Then tell the user where the sanitized file is. It's ready to open locally.
...
- Pairs with `mabl-failure-rca`: that skill exports DOM snapshots as part of its
  artifact pull, and they need sanitizing before you can open them.
