...
metadata:
  ...
  host: kiro
compatibility: >-
  Reads front-end source only. The optional --live pass needs the
  chrome-for-mabl (Chrome DevTools) MCP server connected in Kiro and a running
  dev server.
---
...
| `--live` | off | Also drive the running app via the `chrome-for-mabl` MCP to verify/enrich selectors + capture screenshots |

If the user gives no args, use the defaults and proceed (don't interrogate them).

## Workflow

Run the phases in order. For a large repo, track them as a checklist in your reply
(or as a Kiro spec `tasks.md` if the user is already working inside a spec).
...
- **Personas & credentials** — login page + auth context + any test creds noted in the repo's agent context. In a Kiro workspace that's `.kiro/steering/` (`product.md`, `tech.md`, `structure.md`, or a project-specific steering doc); also check `AGENTS.md` / `README.md`. Capture role → login → what that role can reach.
...
- **UI selector hints & quirks** — `data-testid`s, ARIA labels, `id`s, element roles, and gotchas (e.g. "nav links are `<a>` tags → target by link role, not button"; feature-flagged button label variants). Pull quirks from code + any testing notes in `.kiro/steering/`.
...
Using the `chrome-for-mabl` MCP tools only:
- Start (or reuse) the dev server, then `new_page` → the app, log in with each persona (`fill_form` + `click`), and visit the key routes with `navigate_page`.
- Capture the **real rendered** labels/selectors with `take_snapshot`, and one `take_screenshot` per page.
