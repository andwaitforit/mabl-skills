# mabl-app-crawl — Kiro skill

Kiro port of [`../../claude/mabl-app-crawl`](../../claude/mabl-app-crawl). Same workflow and
same [`crawl-prompt.md`](crawl-prompt.md); only the host wiring differs (MCP config lives in
`.kiro/settings/mcp.json`, connected from Kiro's **MCP Servers** panel).
...
- The **mabl** MCP server added to `.kiro/settings/mcp.json` and connected from Kiro's **MCP
  Servers** panel. If a tool isn't authenticated, the skill stops and asks you to connect it.
