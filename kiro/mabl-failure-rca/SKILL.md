...
metadata:
  ...
  host: kiro
compatibility: >-
  Requires the mabl CLI on PATH and the mabl MCP server connected in Kiro.
  AI failure analysis requires the workspace's AI features to be enabled.
---
...
2. **mabl MCP required** — this skill uses `analyze_mabl_failure`, `get_mabl_test_run_artifact`, etc.
   If the mabl MCP tools aren't available, stop and ask the user to add the mabl server to
   `.kiro/settings/mcp.json` and connect it from Kiro's **MCP Servers** panel.
...
5. **AI entitlement caveat:** `analyze_mabl_failure` / `analyze_mabl_results` require the workspace's AI features to be on. You can't know until you call; if analysis is unavailable, fall back to artifacts + step data and say AI analysis was unavailable.
...
3. **History / "since green" (optional but valuable):** `analyze_mabl_results` on the test run,
   asking what changed since this test last passed — that window scopes the suspect commits in Step 4.
...
⚠️ **Filter before grepping.** Artifact bundles and run logs embed base64 payloads; a naive
grep can dump hundreds of KB into context. Pipe through `awk 'length($0) < 300'` first.

💡 **A saved DOM snapshot may log you out when opened.** The page's own JS re-hydrates and
bounces to login. Use the `mabl-dom-sanitizer` skill to strip executable JS before opening
it locally.
...
- AI failure analysis (`analyze_mabl_failure`, `analyze_mabl_results`) requires the workspace's
  AI features to be enabled; otherwise rely on artifacts + step data and say so.
