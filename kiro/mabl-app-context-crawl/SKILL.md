...
metadata:
  ...
  host: kiro
compatibility: >-
  Requires the chrome-for-mabl (Chrome DevTools) MCP server connected in Kiro,
  a local Chrome, and network access to the target app.
---
...
This skill drives a real browser via the **`chrome-for-mabl`** MCP server (Chrome DevTools MCP). If it isn't connected, tell the user to add it to `.kiro/settings/mcp.json` and connect it from Kiro's **MCP Servers** panel, then stop — do **not** fall back to the source-based skill silently.
...
Tools used: `new_page` / `list_pages` / `select_page`, `navigate_page`, `take_snapshot`, `take_screenshot`, `list_network_requests`, `list_console_messages`, and `fill` / `fill_form` / `click` only when a safe interaction is required (see guardrails).

> **Porting note.** The Claude Code original drives the *Claude in Chrome* MCP server, which has no Kiro equivalent. The tool mapping used here: `list_connected_browsers`/`select_browser` → `list_pages`/`select_page`, `navigate` → `navigate_page`, `read_page`/`get_page_text`/`find` → `take_snapshot`, `read_network_requests` → `list_network_requests`, `read_console_messages` → `list_console_messages`, `form_input` → `fill`/`fill_form`, `computer` screenshot → `take_screenshot`.
...
Track phases as a checklist in your reply (or as a Kiro spec `tasks.md` if the user
is already working inside a spec).

### 1. Connect & confirm scope
- `list_pages` to confirm the browser is reachable; `new_page` to open a working tab. If the MCP server isn't connected, stop and ask the user to connect `chrome-for-mabl`.
...
For each persona: `navigate_page` to `--url`, locate the login form (`take_snapshot`), fill username + password (`fill_form`), submit (`click`), and confirm landing. Logging in is the one expected state change; if a persona's creds fail, note it and continue with the others. Capture the login page's selectors before authenticating.

### 3. Discover routes (BFS link crawl)
From each persona's landing page:
- `take_snapshot` to enumerate nav/sidebar/menu links and their visible text + hrefs.
...
### 4. Capture each page
Per reached page:
- `take_snapshot` for headings, section titles, and the primary purpose.
...
- `take_screenshot` for the page; note the saved reference in the doc.
- Watch `list_console_messages` / `list_network_requests` for obvious errors or the API shape (useful selector/quirk notes) — but never document server internals beyond what the browser exposes.
...
- Clean up: log out the sessions you opened and `close_page` the tabs you opened.
