...
metadata:
  ...
  host: kiro
compatibility: >-
  Requires the mabl MCP server connected in Kiro, with the workspace's AI /
  coverage features enabled. Network access required.
---
...
1. **mabl MCP required** — uses `identify_coverage_gaps` (and `search_mabl_tests` to
   ...
   add the mabl server to `.kiro/settings/mcp.json` and connect it from Kiro's
   **MCP Servers** panel. There is no `mabl agent install kiro` target — the entry is
   written by hand; see this tree's `README.md`.
...
not the documented 5–20. Kiro blocks on a foreground command, so `nohup`-detach the
authoring session and poll the log on a later turn rather than running it inline:

```bash
A_DIR="$HOME/.kiro/tmp/mabl-authoring"; mkdir -p "$A_DIR"
nohup mabl agent authoring initiate ... --mode local --headless --auto-save \
  > "$A_DIR/authoring.log" 2>&1 &
```

Poll with `awk 'length($0) < 300' "$A_DIR/authoring.log" | tail -n 40` — filter by line
length first, these logs embed base64 screenshots.
