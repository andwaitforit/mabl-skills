...
This skill assumes the **mabl** MCP server is connected, plus an **Atlassian** MCP server if the source is Jira/Confluence. If a tool isn't authenticated, tell the user which server to authenticate (`/mcp`) and stop.
